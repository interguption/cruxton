...
confidence:    high
human_input:   true
human_crux:    "Pranav chose to keep the project name in the ledger title via a committed config, rather than drop the name for a neutral title, when the two options were put side by side."
supersedes:    []
...
- **Drop the name entirely; fixed neutral title.** REJECTED (class: constraint) — deterministic, but the ledger loses its project identity, which Pranav wanted to keep.
